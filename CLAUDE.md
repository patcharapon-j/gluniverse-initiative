...
- **Design language ("Etched Glass")**: `docs/design-language.md` is the canonical GL Universe family theming reference (shared with the Clocks & Tracker companion). The CSS `:root` defines the shared `--gl-*` token block (ink ramp, semantic accents incl. `--gl-hazard`/`--gl-signal`, the `--gl-display`/`--gl-tech` voices, and the `--gl-*` motion tokens); the module-local `--gluni-*` tokens **alias** to those, and the themed blocks (`core`/`fantasy`/`chronicle`) re-point the `--gluni-*` aliases — never `--gl-*`. Hero numerals use the Oxanium display voice via `--gluni-display-font`; micro-labels keep the technical (Bahnschrift) voice. When adding tokens, prefer aliasing an existing `--gl-*` value over a new literal.
